...
describe("Address Conversion", async () => {
  ...
  it("Expect legacy address conversion to CashAddr to match reference", () => {
    assert.strictEqual(
      Gen.convertAddress(bchAddresses[0].address, "cashAddress"),
      cashAddress
    );
  });
  it("Expect legacy address conversion to BitPay to match reference", () => {
    assert.strictEqual(
      Gen.convertAddress(bchAddresses[0].address, "bitpayAddress"),
      bitPayAddress
    );
  });
  it("Expect legacy address conversion to BCH SLP to match reference", () => {
    assert.strictEqual(
      Gen.convertAddress(slpAddresses[0].address, "bchSlp"),
      slpAddress
    );
  });
});
