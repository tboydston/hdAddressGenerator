...
if( settings.testCoin == "ALL") {
    ...
    coinData[coinName] = {}
    coinData[coinName] = require("./coins/"+coinName+".js")
...
for (const coin in coinData) {
    ...
    describe(`${ref.longName} (${ref.shortName})`, async () => {
        ...
        try{ bip44Hardened = Gen.withMnemonic(settings.mnemonic,false,ref.shortName,true) } catch(e){}
        try{ bip44Encrypted = Gen.withMnemonic(settings.mnemonic,false,ref.shortName,false,44,0,0,settings.bip38Password) } catch(e){}
        try{ bip44seed = Gen.withSeed(settings.bip32Seed,ref.shortName) } catch(e){}
        try{ bip49 = Gen.withMnemonic(settings.mnemonic,false,ref.shortName,false,49) } catch(e){}
        try{ bip49Encrypted = Gen.withMnemonic(settings.mnemonic,false,ref.shortName,false,49,0,0,settings.bip38Password) } catch(e){}
        try{ bip84 = Gen.withMnemonic(settings.mnemonic,false,ref.shortName,false,84) } catch(e){}
        try{ bip32 = Gen.withMnemonicBIP32(settings.mnemonic,false,ref.shortName,"m/0'/0'",false) } catch(e){}
        ...
        if ( ref.bip32RootKeyBip44WithPassphrase != undefined ){
            it('Expect bip32RootKeyBip44 to match reference when passphrase used.', () => {
                assert.strictEqual(bip44WithPassPhrase.bip32RootKey,ref.bip32RootKeyBip44WithPassphrase)
            })
        }
        if ( ref.bip32RootKeyBip49 != undefined ){
            it('Expect bip32RootKeyBip141 to match reference.', () => {
        ...
        if ( ref.addressBip44index0 != undefined && ref.addressBip44index1 != undefined ){
            it('Expect addressBip44index0 and addressBip44index1 address, pub, and priv keys to match reference.', async () => {
                let addresses = await bip44.generate(2)
        ...
        if ( ref.addressBip44index0 != undefined && ref.addressBip44index1 != undefined && bip44seed != false ){
            it('Expect addressBip44index0 and addressBip44index1 address when generated with seed, pub, and priv keys to match reference.', async () => {
                let addresses = await bip44seed.generate(2)
        ...
        if ( ref.addressBip44HardIndex0 != undefined ){
            it('Expect addressBip44HardIndex0 hardened address, pub, and priv keys to match reference.', async () => {
                let addresses = await bip44Hardened.generate(1)
        ...
        if ( ref.addressBip44EncryptedIndex0 != undefined ){        
            it('Expect bip44Encrypted encrypted address, pub, and priv keys to match reference.', async () => {
                let addresses = await bip44Encrypted.generate(1)
        ...
        if ( ref.addressBip49index0 != undefined && ref.addressBip49index1 != undefined ){         
            it('Expect addressBip49index0 and addressBip49index1 address, pub, and priv keys to match reference.', async () => {
                let addresses = await bip49.generate(2)
        ...
        if ( ref.addressBip49EncryptedIndex0 != undefined ){  
            it('Expect bip49Encrypted encrypted address, pub, and priv keys to match reference.', async () => {
                let addresses = await bip49Encrypted.generate(1)
        ...
        if ( ref.addressBip84index0 != undefined && ref.addressBip84index1 != undefined ){ 
            it('Expect addressBip84index0 and addressBip84index1 address, pub, and priv keys to match reference.', async () => {
                let addresses = await bip84.generate(2)
        ...
        if ( ref.addressBip32index0 != undefined && ref.addressBip32index1 != undefined ){ 
            it('Expect addressBip32index0 and addressBip32index1 address, pub, and priv keys to match reference.', async () => {
                let addresses = await bip32.generate(2)
        ...
        if ( ref.addressBip32index0 != undefined && ref.addressBip32index1 != undefined && bip32seed != false ){ 
            it('Expect addressBip32index0 and addressBip32index1 address with seed instead of mnemonic, pub, and priv keys to match reference.', async () => {
                let addresses = await bip32seed.generate(2)
        ...
        if ( ref.addressBip141index0 != undefined && ref.addressBip141index1 != undefined ){ 
            it('Expect addressBip32index0 and addressBip32index1 address, pub, and priv keys to match reference.', async () => {
                let addresses = await bip141.generate(2)
        ...
        if ( ref.addressBip141index0 != undefined && ref.addressBip141index1 != undefined && bip141seed != false ){         
            it('Expect addressBip32index0 and addressBip32index1 address with seed instead of mnemonic, pub, and priv keys to match reference.', async () => {
                let addresses = await bip141seed.generate(2)
